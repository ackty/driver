...
static int  __init  device_register_unregister_init(void);         //模块加载函数声明
static void  __exit device_register_unregister_exit(void);         //模块卸载函数声明
...
int __init device_register_unregister_init(void)
{
    ...
    res=device_register(dev);               //将新创建的逻辑设备加入Linux内核系统
    if(res)
    {
        printk("register device failed\n");
        device_unregister(dev);             //失败，则注销逻辑设备
        kfree(dev);                         //失败，释放逻辑设备的内存空间
        return -1;
    }
    printk("register device success\n");
    printk("out device_register_unregister_init\n");
    return 0;
}

void __exit device_register_unregister_exit (void)
{
    printk("into device_register_unregister_exit\n");
    ...
    printk("out device_register_unregister_exit\n");
}
...
module_init(device_register_unregister_init);               //模块加载
module_exit(device_register_unregister_exit);               //模块卸载
